...
for b in [(535,25,230,125,''),(530,160,170,195,'主屋'),(405,385,230,80,'缘侧'),
          (310,260,160,100,'厨房'),(125,500,275,80,'手合场'),(125,580,95,160,''),
          (330,580,100,160,''),(60,330,105,115,''),(210,305,85,90,''),
          (185,25,280,55,''),(890,25,85,45,''),(1000,25,85,45,''),
          (500,465,80,70,''),(485,555,30,30,''),
          (210,855,200,80,''),(450,755,150,45,''),(450,800,50,75,''),
          (825,205,90,45,''),
          (1030,840,45,48,'')]:
    building(*b)

# White threshold strips and the smaller southwest buildings from the plan.
for x,y,w,h in [(310,260,18,100),(452,292,18,68),(330,722,100,18),
                (210,855,20,80),(390,855,20,80)]:
    rect(x,y,w,h,'eee9db')
# Repeated bays make the long wings read as multiple rooms at this scale.
for y in range(610, 721, 32):
    line([(130,y),(215,y)],'b49973',2,-4)
    line([(335,y),(425,y)],'b49973',2,-4)
for x in range(250, 390, 40):
    line([(x,877),(x,930)],'b49973',2,-4)
...
destinations=[('Main',710,300,'FromCourtyard','主屋'),('Kitchen',480,325,'FromMain','厨房'),
              ...
              ('Field',190,255,'FromCourtyard','田地')]
